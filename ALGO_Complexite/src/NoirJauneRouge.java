...
public class NoirJauneRouge {
	...
	public boolean ajouter(String nom, char couleur){
		...
		// TODO
		if ( NOMBRE_MAX_ECOLIERS == tableTriee.length -1) {
			return false;
		}else {
			if (trouverIndiceEcolier(nom) != -1) {
				...
				for (int i = tableTriee.length -1; i >=0 ; i--) {
					if (ecolier.getCouleur() == 'r'){
						tableTriee[i] = ecolier;
						nombreRouges++;
						return true;
					}else if (ecolier.getCouleur() == 'j' ){
						trouverIndiceEcolier(nom);
						tableTriee[i] = tableTriee[nombreEcoliers() - nombreRouges];
						tableTriee[nombreNoirs + nombreJaunes ] = ecolier;
						nombreJaunes++;
						return true;
					}else if (ecolier.getCouleur() == 'n'){
						tableTriee[i] =tableTriee[nombreEcoliers() -nombreRouges];
						 tableTriee[nombreEcoliers() - nombreRouges] = tableTriee[nombreNoirs ] ;
	...
	public boolean supprimer(String nom) {
		...
		if (trouverIndiceEcolier(nom) == -1){
			...
			for (int i = trouverIndiceEcolier(nom); i >= 0  ; i--) {
				if (tableTriee[i].getCouleur() == 'r'){
					tableTriee[i] = tableTriee[i + nombreRouges -1];
					tableTriee[i + nombreRouges -1] = null;
					nombreRouges--;
					return true;
				} else if (tableTriee[i].getCouleur() == 'j') {
					tableTriee[i] = tableTriee[i + nombreJaunes -1];
					tableTriee[i + nombreJaunes -1] = tableTriee[nombreEcoliers()-1];
					tableTriee[nombreEcoliers()-1] = null;
					nombreJaunes--;
					return true;
				} else if (tableTriee[i].getCouleur() == 'n') {
					tableTriee[i] = tableTriee[nombreNoirs -1];
					tableTriee[nombreNoirs - 1] = tableTriee[nombreEcoliers() - nombreRouges -1];
					tableTriee[nombreEcoliers() -nombreRouges-1] = tableTriee[nombreEcoliers() -1];
					tableTriee[nombreEcoliers() -1] = null;
					nombreNoirs--;
					return true;
