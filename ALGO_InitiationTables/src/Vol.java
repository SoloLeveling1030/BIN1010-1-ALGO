...
public class Vol {
	...
	public boolean estUnTour(){
		...
		//TODO
		return tableCoordonnees[0].equals(lieuArrivee());
	}
	...
	public Coordonnees lieuLePlusEloigne(){
		...
		Coordonnees lieuEloigne=tableCoordonnees[0];

		for (int i = 1; i < tableCoordonnees.length; i++) {
			if (lieuDepart.distance(tableCoordonnees[i]) > lieuDepart.distance(lieuEloigne) ){
				lieuEloigne = tableCoordonnees[i];
			}
		}
